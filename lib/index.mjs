...
function convertNodes(nodes, deco) {
...
function createParagraph(node, deco) {
...
function createHeading(node, deco) {
...
function createBlockquote(node, deco) {
    return {
        type: node.type,
        children: convertNodes(node.children, deco),
    };
}
function createList(node, deco) {
...
function createListItem(node, deco) {
...
function createTable(node, deco) {
...
function createTableRow(node, deco) {
    var type = node.type, children = node.children, header = node.header, footer = node.footer;
    return {
        type: type,
        header: header,
        footer: footer,
...
function createTableCell(node, deco) {
    var type = node.type, children = node.children, header = node.header, index = node.index;
    return {
        type: type,
        header: header,
        index: index,
...
function createFootnoteDefinition(node, deco) {
...
function createLinkReference(node, deco) {
...
function createFootnote(node, deco) {
    var type = node.type, children = node.children;
    return {
        type: type,
        children: convertNodes(children, deco),
...
export { plugin as remarkToSlate, plugin$3 as remarkToSlateLegacy, plugin$1 as slateToRemark, plugin$2 as slateToRemarkLegacy };
